...
class BhujApp:
    ...
    def _tab_params(self, parent):
        ...
        tk.Entry(nm_f, textvariable=self.pv['case_name'], bg='#0D1B2A', fg='#FFE082',
                 font=('Helvetica',10,'bold'), width=55, relief=tk.FLAT,
        ...
        tk.Label(tp_lf, text='[Standard: a=0.108, b=15.5939, Chopra 2010]\nT_path is the path duration (travel time component).\nSource duration T_src = 1/fc; total window = T_src + T_path.',
                 bg='#1E2A3A', fg='#A5D6A7', font=('Courier',7), justify=tk.LEFT).grid(
        ...
        tk.Label(seed_lf, text=seed_info, bg='#0D1B2A', fg='#A5D6A7',
                 font=('Courier',7), justify=tk.LEFT, padx=6, pady=4).grid(
                     row=2, column=0, columnspan=2, padx=4, sticky='w')

        # Derived values
        self._drv = tk.Label(c2, text='', bg='#0D1B2A', fg='#64B5F6',
                              font=('Courier',7), justify=tk.LEFT, padx=4, pady=4)
...
"""
    ...
    def _litho_editor(self, title, layer_list, hdrs):
        ...
        tk.Label(w, text=(
            ...
            'Chandrasekhar et al. (2004) GRL; NGRI (2001) Technical Report; Rajendran & Rajendran (2001) Curr.Sci.'),
                 bg='#1E2A3A', fg='#A5D6A7', font=('Courier',7), justify=tk.LEFT).pack(padx=8, pady=4)
        def apply_edit():
            try:
    ...
    def _tab_simulate(self, parent):
        ...
        for m, lbl in MLB.items():
            ...
            tk.Label(fr, text=f'  {FC_EQS[m]}', bg='#1E2A3A', fg='#A5D6A7',
        ...
        tk.Label(seed_f, text='Each seed = 1 random realization.\nMEDIAN of all realizations used.\nMore seeds = more stable result.',
                 bg='#1E2A3A', fg='#A5D6A7', font=('Courier',7), justify=tk.LEFT).pack(padx=6, pady=2)
...
""")
    ...
    def _build_vs_panel(self, parent):
        ...
        self._calc_log=tk.Label(calc_lf,text='',bg='#1E2A3A',fg='#A5D6A7',font=('Courier',7))
        ...
        tk.Button(calc_lf,text='▶ Calculate RS from Station TH',
                   ...
                   bg='#D35400',fg='white',font=('Helvetica',10,'bold'),relief=tk.FLAT,padx=14,pady=6).pack(pady=6)
    ...
    def _tab_report(self, parent):
        ...
        tk.Button(ctl,text='📄 HTML Report',command=self._gen_html,bg='#27AE60',fg='white',font=('Helvetica',10,'bold'),relief=tk.FLAT,padx=14,pady=6).pack(side=tk.LEFT,padx=8)
        tk.Button(ctl,text='📝 MD Report',command=self._gen_md,bg='#D35400',fg='white',font=('Helvetica',10,'bold'),relief=tk.FLAT,padx=14,pady=6).pack(side=tk.LEFT,padx=4)
        tk.Button(ctl,text='📐 Equations HTML',command=self._gen_eq_html,bg='#8E44AD',fg='white',font=('Helvetica',10,'bold'),relief=tk.FLAT,padx=14,pady=6).pack(side=tk.LEFT,padx=4)
